...
const SearchConfig = {
  ...
  placeholder: 'Tìm kiếm nhanh... [Ctrl+K, cmd+K]',
  classNames: {
    detachedContainer: 'd-flex flex-column admin-menu-search-modal',
    ...
    panel: 'flex-grow content-wrapper overflow-hidden position-relative admin-menu-search-panel',
    panelLayout: 'h-100',
    clearButton: 'd-none',
    item: 'd-block'
  }
...
function menuItemMatchesQuery(item, query) {
  if (!query) return true;
  const q = normalizeSearchText(query);
  const haystack = normalizeSearchText([item.name, item.section].filter(Boolean).join(' '));
  return haystack.includes(q);
}

function buildSearchDataFromMenuItems(items) {
  const navigation = {};
...
function loadSearchData() {
  data = buildSearchDataFromMenuItems(window.adminMenuSearchItems || []);
  initializeAutocomplete();
}

function menuSearchItemLink(item, html) {
  return html`
    <a href="${item.url}" class="suggestion-item d-flex align-items-center admin-menu-search-link">
      <i class="icon-base ti ${item.icon}"></i>
      <span class="admin-menu-search-label">${item.name}</span>
    </a>
  `;
}

function menuSearchItemsGrid(items, html, renderCell) {
  return html`
    <div class="row g-2 admin-menu-search-items-grid">
      ${items.map(item => html`<div class="col-12 col-sm-6 col-lg-4">${renderCell(item)}</div>`)}
    </div>
  `;
}
    ...
    render(args, root) {
      ...
      if (!state.query) {
        const initialSuggestions = html`
          <div class="admin-menu-search-scroll px-3 px-lg-4 py-3">
            ${Object.entries(data.suggestions || {}).map(
              ([section, items]) => html`
                <div class="admin-menu-search-section suggestion-section">
                  <p class="search-headings mb-2">${section}</p>
                  ${menuSearchItemsGrid(items, html, item => menuSearchItemLink(item, html))}
                </div>
              `
            )}
          </div>
      ...
      render(html`<div class="admin-menu-search-scroll admin-menu-search-scroll--results px-2 py-2">${children}</div>`, root);
    },
    getSources() {
      if (!data.navigation) {
        return [];
      ...
      return Object.keys(data.navigation).map(section => ({
        sourceId: `nav-${section}`,
        getItems({ query }) {
          const items = data.navigation[section].map(item => ({ ...item, section }));
          return items.filter(item => menuItemMatchesQuery(item, query));
        },
        getItemUrl({ item }) {
          return item.url;
        },
        templates: {
          header({ items, html }) {
            ...
            return html`<span class="search-headings admin-menu-search-section-heading">${section}</span>`;
          },
          item({ item, html }) {
            return menuSearchItemLink(item, html);
          }
        }
      }));
    }
